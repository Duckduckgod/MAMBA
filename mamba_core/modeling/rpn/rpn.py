...
from mamba_core.modeling.box_coder import BoxCoder
from mamba_core.modeling.rpn.retinanet.retinanet import build_retinanet
...
class RPNHeadConvRegressor(nn.Module):
    ...
    def __init__(self, cfg, in_channels, num_anchors):
        ...
        for _l in [self.cls_logits, self.bbox_pred]:
...
class RPNHead(nn.Module):
    ...
    def __init__(self, cfg, in_channels, num_anchors):
        ...
        for _l in [self.conv, self.cls_logits, self.bbox_pred]:
            torch.nn.init.normal_(_l.weight, std=0.01)
            torch.nn.init.constant_(_l.bias, 0)
...
class RPNModule(torch.nn.Module):
    ...
    Takes feature maps from the backbone and outputs RPN proposals and losses.
...
class RPNWithRefModule(RPNModule):
    ...
    def __init__(self, cfg, in_channels):
        ...
        rpn_box_coder = BoxCoder(weights=(1.0, 1.0, 1.0, 1.0))
        box_selector_ref = make_rpn_postprocessor(
            cfg, rpn_box_coder, is_train=True, version="ref"
        )
...
def build_rpn(cfg, in_channels):
    """This gives the gist of it.

    Not super important because it doesn't change as much
    """
    ...
    if cfg.MODEL.VID.ENABLE:
        FUNC_DICT = {
            ...
            "selsa": RPNWithRefModule,
            "mamba": RPNWithRefModule,
        }
