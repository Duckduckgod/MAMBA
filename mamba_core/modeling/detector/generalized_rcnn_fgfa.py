...
from ..backbone import build_backbone, build_embednet, build_flownet
...
class GeneralizedRCNNFGFA(nn.Module):
    ...
    def compute_weight(self, embed_ref, embed_cur):
        ...
        weight = torch.sum(
            embed_ref_normalized * embed_cur_normalized, dim=1, keepdim=True
        )
    ...
    def _forward_train(self, img, imgs_ref, targets):
        ...
        concat_imgs = torch.cat(
            [img.tensors, *[img_ref.tensors for img_ref in imgs_ref]], dim=0
        )
    ...
    def _forward_test(self, imgs, infos, targets=None):
        ...
        # compute weights
        embed_cur = embeds[
            self.key_frame_location : self.key_frame_location + 1, :, :, :
        ]
