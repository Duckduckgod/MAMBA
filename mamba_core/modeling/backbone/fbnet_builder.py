...
from mamba_core.layers import BatchNorm2d, Conv2d, FrozenBatchNorm2d, interpolate
from mamba_core.layers.misc import _NewEmptyTensorOp
...
class ChannelShuffle(nn.Module):
    ...
    def forward(self, x):
        """Channel shuffle: [N,C,H,W] -> [N,g,C/g,H,W] -> [N,C/g,g,H,w] ->
...
class FBNetBuilder:
    ...
    def add_last(self, stage_info):
        """skip last layer if channel_scale == 0 use the same output channel if
        channel_scale < 0."""
        assert len(stage_info) == 2
