...
from mamba_core.modeling import poolers, registry
from mamba_core.modeling.rpn import rpn

from . import fbnet_builder as mbuilder
from . import fbnet_modeldef as modeldef
...
def create_builder(cfg):
    ...
    if rpn_stride is not None:
        ...
        ), f"Needs to set cfg.MODEL.RPN.ANCHOR_STRIDE to {rpn_stride}, got {cfg.MODEL.RPN.ANCHOR_STRIDE}"
    ...
    logger.info(f"Building fbnet model with arch {arch} (without scaling):\n{arch_def}")
...
def _get_rpn_stage(arch_def, num_blocks):
    ...
    if num_blocks > 0:
        logger.warn(f"Use last {num_blocks} blocks in {ret} as rpn")
        block_count = len(ret["stages"])
        assert (
            num_blocks <= block_count
        ), f"use block {num_blocks}, block count {block_count}"
