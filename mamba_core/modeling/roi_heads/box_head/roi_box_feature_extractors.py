...
from mamba_core.modeling import registry
from mamba_core.modeling.backbone import resnet
from mamba_core.modeling.make_layers import Conv2d, group_norm, make_fc
from mamba_core.modeling.mem_bank.memory_bank_ins import MemoryBankIns
from mamba_core.modeling.poolers import Pooler
from mamba_core.structures.boxlist_ops import cat_boxlist
...
@registry.ROI_BOX_FEATURE_EXTRACTORS.register("MAMBAFeatureExtractor")
class MAMBAFeatureExtractor(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        if cfg.MODEL.VID.MAMBA.INS_MEM.ENABLE:
            self.ins_mem = MemoryBankIns(cfg)
            self.num_enhancement = cfg.MODEL.VID.MAMBA.INS_MEM.NUM_ENHANCEMENT
            self.num_ref = cfg.MODEL.VID.RPN.REF_POST_NMS_TOP_N
            ...
            self.fcs = nn.ModuleList(
                [
                    make_fc(input_size, representation_size, use_gn),
                    make_fc(representation_size, representation_size, use_gn),
                ]
            )
    ...
    def _forward_train(self, x, proposals):
        ...
        proposals, proposals_refs = proposals[0][0], proposals[1:]
        ...
        if proposals_refs:
            ...
            # VID image data
            x_refs = x  # self attention

        # enhance with instance-level memory
        for i in range(self.num_enhancement):
            x = F.relu(self.fcs[i](x))
            x_refs = F.relu(self.fcs[i](x_refs))
            x = self.ins_mem.geos[i](x, x_refs)
...
class AttentionExtractor(nn.Module):
    ...
    def extract_position_embedding(position_mat, feat_dim, wave_length=1000.0):
        ...
        dim_mat = torch.full((len(feat_range),), wave_length, device=device).pow(
            8.0 / feat_dim * feat_range
        )
...
class RDNFeatureExtractor(AttentionExtractor):
    ...
    def __init__(self, cfg, in_channels):
        ...
        if cfg.MODEL.VID.ROI_BOX_HEAD.ATTENTION.ENABLE:
            ...
            for i in range(self.base_stage + self.advanced_stage + 1):
                ...
                Wgs.append(
                    Conv2d(
                        self.embed_dim, self.groups, kernel_size=1, stride=1, padding=0
                    )
                )
                ...
                for _l in [Wgs[i], Wvs[i]]:
                    torch.nn.init.normal_(_l.weight, std=0.01)
                    torch.nn.init.constant_(_l.bias, 0)
            self.fcs = nn.ModuleList(fcs)
    ...
    def _forward_train(self, x, proposals):
        ...
        proposals, proposals_cur, proposals_refs = (
            proposals[0][0],
            proposals[1],
            proposals[2:],
        )
...
@registry.ROI_BOX_FEATURE_EXTRACTORS.register("SELSAFeatureExtractor")
class SELSAFeatureExtractor(AttentionExtractor):
    def __init__(self, cfg, in_channels):
        super().__init__(cfg, in_channels)
...
class MEGAFeatureExtractor(AttentionExtractor):
    ...
    def init_memory(self):
        ...
        for i in range(self.stage):
            queue = {
                "rois": deque(maxlen=self.all_frame_interval),
    ...
    def update_global(self, feats):
        ...
        self.global_cache[0]["feats"] = torch.cat(
            list(self.global_queue_list[0]["feats"]), dim=0
        )
    ...
    def update_memory(self, i, cache):
        ...
        self.mem[i] = {
            "rois": torch.cat(list(self.mem_queue_list[i]["rois"]), dim=0),
    ...
    def generate_feats(self, x, proposals, proposals_key=None, ver="local"):
        ...
        if ver in ("local", "memory"):
            x_dis = torch.cat(
                [x[: self.advanced_num] for x in torch.split(x, self.base_num, dim=0)],
                dim=0,
            )
            rois_dis = torch.cat(
                [
        ...
        if ver == "memory":
            ...
            self.local_cache.append(
                {
                    "rois_cur": torch.cat([rois_key, rois_dis], dim=0),
                    "rois_ref": rois,
    ...
    def _forward_train(self, x, proposals):
        ...
        with torch.no_grad():
            if self.memory_cache:
                self.mem = []
                for i in range(self.stage):
                    feats = self._forward_train_single(
                        i, self.memory_cache[i], None, ver="memory"
                    )
        ...
        for i in range(self.stage):
            ...
            feats = self._forward_train_single(
                i, self.local_cache[i], memory, ver="local"
            )
