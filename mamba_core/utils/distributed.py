...
def get_gpus_nocache():
    ...
    with run_and_terminate_process(
        cmds, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=1
    ) as process:
...
def gpu_indices(divisible=True):
    ...
    assert (
        0 <= local_rank < local_size
    ), f"Invalid local_rank: {local_rank} local_size: {local_size}"
    gpu_count = len(get_gpus())
    assert (
        gpu_count >= local_size > 0
    ), f"GPU count: {gpu_count} must be >= LOCAL_SIZE: {local_size} > 0"
    if divisible:
        ngpu = int(gpu_count / local_size)
        gpus = np.arange(local_rank * ngpu, (local_rank + 1) * ngpu)
        if gpu_count % local_size != 0:
            logging.warning(
                "gpu_count: {} not divisible by local_size: {}; "
                + f'{"some GPUs may be unused"}'
            )
    else:
