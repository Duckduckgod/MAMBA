...
def _vc_home():
    ...
    """
    home = os.environ.get(
        "PHILLY_VC_NFS_DIRECTORY", os.environ.get("PHILLY_VC_DIRECTORY")
    )
    if not home:
        home = op.expanduser("~")
...
def job_id(path=None):
    ...
    m = re.search(r"/(?P<app_id>application_[\d_]+)[/\w]*$", path)
    if m:
...
def get_master_ip(master_name=None):
    ...
    print(f"name_ip_pairs: {name_ip_pairs}")
    ...
    for name_ip_pair in name_ip_pairs:
        ...
        value = pair_list[0]
        name2ip[key] = value
        print(f"{name_ip_pair}, key: {key}, value: {value}")
