...
from mamba_core.utils.comm import get_world_size
from mamba_core.utils.imports import import_file
from mamba_core.utils.miscellaneous import save_labels
...
def build_dataset(
    dataset_list, transforms, dataset_catalog, is_train=True, method="base"
):
    """
    Arguments:
...
def make_data_sampler(dataset, shuffle, distributed, method="base", is_train=True):
    if distributed:
        ...
        elif method in ("rdn", "mega", "fgfa", "dff", "selsa", "mamba"):
            if is_train:
                return samplers.DistributedSampler(dataset, shuffle=shuffle)
            else:
...
def make_data_loader(
    ...
    paths_catalog = import_file(
        "mamba_core.config.paths_catalog", cfg.PATHS_CATALOG, True
    )
    ...
    for dataset in datasets:
        sampler = make_data_sampler(
            dataset, shuffle, is_distributed, cfg.MODEL.VID.METHOD, is_train
        )
        ...
        collator = (
            ...
            if not is_train and cfg.TEST.BBOX_AUG.ENABLED
                ...
                cfg.DATALOADER.SIZE_DIVISIBILITY, cfg.MODEL.VID.METHOD, is_train
            )
        )
