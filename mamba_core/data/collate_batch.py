...
class BatchCollator:
    """From a list of samples from the dataset, returns the batched images and
    targets.
    ...
    def __call__(self, batch):
        ...
        elif self.method in ("rdn", "mega", "fgfa", "dff", "selsa", "mamba"):
            assert (
                len(transposed_batch[0]) == 1
            ), "Currently 1 gpu could only hold 1 image. Please modify SOLVER.IMS_PER_BATCH and TEST.IMS_PER_BATCH to ensure this."
            images = {}
...
class BBoxAugCollator:
    """From a list of samples from the dataset, returns the images and targets.
