...
from mamba_core.utils.comm import is_main_process
...
class VIDDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if self.is_train:
            ...
            if len(lines[0]) == 2:
                self.image_set_index = [
                    self.image_set_index[idx] for idx in range(len(keep)) if keep[idx]
                ]
                self.frame_id = [
                    self.frame_id[idx] for idx in range(len(keep)) if keep[idx]
                ]
            else:
                self.image_set_index = [
                    self.image_set_index[idx] for idx in range(len(keep)) if keep[idx]
                ...
                self.pattern = [
                    self.pattern[idx] for idx in range(len(keep)) if keep[idx]
                ]
                self.frame_id = [
                    self.frame_id[idx] for idx in range(len(keep)) if keep[idx]
                ]
                self.frame_seg_id = [
                    self.frame_seg_id[idx] for idx in range(len(keep)) if keep[idx]
                ]
                self.frame_seg_len = [
                    self.frame_seg_len[idx] for idx in range(len(keep)) if keep[idx]
                ]
        ...
        self.annos = self.load_annos(
            os.path.join(self.cache_dir, self.image_set + "_anno.pkl")
        )
    ...
    def filter_annotation(self):
        ...
        if os.path.exists(cache_file):
            ...
            if is_main_process():
                print(f"{self.det_vid}'s keep information loaded from {cache_file}")
            return keep

        keep = np.zeros((len(self)), dtype=np.bool)
        for idx in range(len(self)):
            if idx % 10000 == 0:
                print(f"Had filtered {idx} images")
        ...
        if is_main_process():
            ...
            print(f"Saving {self.det_vid}'s keep information into {cache_file}")
    ...
    def _preprocess_annotation(self, target):
        ...
        for obj in objs:
            ...
            boxes.append(box)
            gt_classes.append(
                self.classes_to_ind[obj.find("name").text.lower().strip()]
            )
    ...
    def load_annos(self, cache_file):
        if os.path.exists(cache_file):
            ...
            if is_main_process():
                print(
                    f"{self.det_vid}'s annotation information loaded from {cache_file}"
                )
        else:
            annos = []
            for idx in range(len(self)):
                if idx % 10000 == 0:
                    print(f"Had processed {idx} images")
            ...
            if is_main_process():
                ...
                print(
                    f"Saving {self.det_vid}'s annotation information into {cache_file}"
                )
