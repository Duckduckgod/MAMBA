...
class VIDFGFADataset(VIDDataset):
    ...
    def _get_train(self, idx):
        ...
        if hasattr(self, "pattern"):
            offsets = (
                np.random.choice(
                    cfg.MODEL.VID.FGFA.MAX_OFFSET - cfg.MODEL.VID.FGFA.MIN_OFFSET + 1,
                    cfg.MODEL.VID.FGFA.REF_NUM,
                    replace=False,
                )
                + cfg.MODEL.VID.FGFA.MIN_OFFSET
            )
    ...
    def _get_test(self, idx):
        ...
        ref_id = min(
            self.frame_seg_len[idx] - 1, frame_id + cfg.MODEL.VID.FGFA.MAX_OFFSET
        )
        ref_filename = self.pattern[idx] % ref_id
        img_ref = Image.open(self._img_dir % ref_filename).convert("RGB")
        img_refs.append(img_ref)
