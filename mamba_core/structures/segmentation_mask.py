...
from mamba_core.utils import cv2_util
...
class BinaryMaskList:
    ...
    def __init__(self, masks, size):
        ...
        assert masks.shape[1] == size[1], "{} != {}".format(masks.shape[1], size[1])
        assert masks.shape[2] == size[0], "{} != {}".format(masks.shape[2], size[0])
...
class PolygonInstance:
    """This class holds a set of polygons that represents a single instance of
    an object mask.
