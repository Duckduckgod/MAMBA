...
import torch
import unittest
from mamba_core.config import cfg as g_cfg
from mamba_core.modeling import registry

# import modules to to register predictors
from mamba_core.modeling.backbone import build_backbone  # NoQA
from mamba_core.modeling.roi_heads.roi_heads import build_roi_heads  # NoQA
...
class TestPredictors(unittest.TestCase):
    def test_roi_box_predictors(self):
        ...
        for cur_in, cur_out, cur_cfg in _test_predictors(
            ...
            self.assertEqual(scores.shape[1], cur_cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES)
    ...
    def test_roi_mask_predictors(self):
        ...
        for cur_in, cur_out, cur_cfg in _test_predictors(
            ...
            self.assertEqual(cur_out.shape[0], cur_in.shape[0])
            self.assertEqual(cur_out.shape[1], cur_cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES)
