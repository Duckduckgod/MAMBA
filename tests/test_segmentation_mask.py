...
import torch
import unittest
from mamba_core.structures.segmentation_mask import SegmentationMask
...
class TestSegmentationMask(unittest.TestCase):
    ...
    def test_transpose(self):
        ...
        FLIP_TOP_BOTTOM = 1
        diff_hor = self.L1(
            self.M.transpose(FLIP_LEFT_RIGHT), self.P.transpose(FLIP_LEFT_RIGHT)
        )

        diff_ver = self.L1(
            self.M.transpose(FLIP_TOP_BOTTOM), self.P.transpose(FLIP_TOP_BOTTOM)
        )
