...
class VideoProcessor:
    ...
    def get_frame(self, frame_id):
        ...
        if frame_id < 0 or frame_id >= self._frame_cnt:
            raise IndexError(
                '"frame_id" must be between 0 and {}'.format(self._frame_cnt - 1)
            )
    ...
    def cvt2frames(
        self, frame_dir, file_start=0, filename_tmpl="{:06d}.jpg", start=0, max_num=0
    ):
        ...
        for i in range(task_num):
            ...
            if img is None:
                break
            filename = os.path.join(frame_dir, filename_tmpl.format(i + file_start))
    ...
    def frames2videos(self, frames, output_folder):
        ...
        videoWriter = cv2.VideoWriter(
            os.path.join(output_folder, self._output_video_name),
            fourcc,
            self._fps,
            (width, height),
        )
...
class VIDDemo:
    ...
    def build_pil_transform(self):
        """Creates a basic transformation that was used in
        generalized_rnn_{}._forward_test()"""
        cfg = self.cfg
        ...
        for frame_id in tqdm(range(frame_seg_len)):
            ...
            if self.method == "base":
                ...
                if self.method == "dff":
                    ...
                    ref_id = min(
                        frame_seg_len - 1, frame_id + self.cfg.MODEL.VID.MEGA.MAX_OFFSET
                    )
                    ref_filename = pattern % ref_id
                    img_ref = cv2.imread(img_dir % ref_filename)
                    img_ref = self.perform_transform(img_ref)
                    ...
                    if self.cfg.MODEL.VID.MEGA.GLOBAL.ENABLE:
                        size = (
                            self.cfg.MODEL.VID.MEGA.GLOBAL.SIZE if frame_id == 0 else 1
                        )
                        ...
                        for id in range(size):
                            filename = (
                                ...
                                % shuffled_index[
                                    (
                                        frame_id
                                        + self.cfg.MODEL.VID.MEGA.GLOBAL.SIZE
                                        - id
                                        - 1
                                    )
                                    % frame_seg_len
    ...
    def generate_images(self, visualization_results):
        for frame_id in range(len(visualization_results)):
            cv2.imwrite(
                os.path.join(self.output_folder, "%06d.jpg" % frame_id),
                visualization_results[frame_id],
            )
